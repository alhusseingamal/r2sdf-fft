...
module sdf_stage_d2 #(parameter N = 8, parameter W = 16, parameter F = 14, parameter STAGE_INDEX = 0) (
...
// 1. Register butterfly and flush outputs (Cycle 1)
reg signed [W-1:0] flush_real_r, flush_imag_r;
reg                twiddle_sel_r;
reg                valid_pipe_r;
...
always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        ...
        flush_real_r   <= flush_real_s;
        flush_imag_r   <= flush_imag_s;
        twiddle_sel_r  <= (!select_counter_msb) && counter[0];
        valid_pipe_r   <= 1'b1;
    end else begin
        valid_pipe_r   <= 1'b0;
    end
...
// 2. Perform twiddle swap/negate on registered signals and register to output (Cycle 2)
wire signed [W-1:0] mul_out_real = (twiddle_sel_r) ?  flush_imag_r : flush_real_r;
wire signed [W-1:0] mul_out_imag = (twiddle_sel_r) ? -flush_real_r : flush_imag_r;
...
always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        ...
        out_imag  <= 'sd0;
        valid_out <= 1'b0;
    end else if (valid_pipe_r) begin
