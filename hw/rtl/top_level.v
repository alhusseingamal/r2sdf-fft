...
wire signed [NUM_STAGES:0][W-1:0] stage_real;
wire signed [NUM_STAGES:0][W-1:0] stage_imag;
...
generate
    for (i = 0; i < NUM_STAGES; i = i+1) begin : stage_gen
        sdf_stage #(.N(N), .W(W), .F(F), .STAGE_INDEX(i)) stage_inst(
            ...
            .in_real(stage_real[i]), .in_imag(stage_imag[i]), 
            .valid_out(stage_valid[i+1]), 
            .out_real(stage_real[i+1]), .out_imag(stage_imag[i+1])
        );
    end
endgenerate
